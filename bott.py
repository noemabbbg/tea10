...
async def subchanneldone(message: types.Message):
    await bot.send_message('/broadcastboxer /broadcastsuicideboy /broadcastsuicideboy /broadcastbastard /broadcastantifanatka /broadcastqueenwithscalpel /broadcastodnazhprinc /broadcastchertovka /broadcast1337 /sheesh /start')
...
async def process_video_command(call: CallbackQuery):
    ...
    if check_sub_channel(await bot.get_chat_member(chat_id=channel_id, user_id=call.from_user.id)):
        await bot.delete_message(call.from_user.id, call.message.message_id)
        await call.message.answer('Я стала злодейкой любовного романа. Думаете, меня что-то не устраивает? Нет, всё просто прекрасно. Статус дочери герцога даёт возможность жить в роскоши и комфорте, и я собираюсь извлечь всю выгоду из своего нынешнего положения. Но, хотя я не хочу идти по пути антагонистки, белой и пушистой тоже не буду. Мой жених изменил мне с главной героиней, поэтому я перепишу оригинальную историю и поставлю всех на колени. Эти ублюдки не стоят моих слёз. оценка на мангалибе: 4.83', reply_markup=clavaChangeState)
